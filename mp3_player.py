...
def get_time():
    ...
    song = playlist[(listbox.curselection())[0]][1]
    song_mut = MP3(song)
    song_length = int(song_mut.info.length)
    if paused:
        pass
    else:
        if song_length == total_seconds:
...
volume_slider = ttk.Scale(root, from_=0, to=1, orient=VERTICAL, value=1, command=volume, length=150)
volume_slider.grid(row=0, column=6, columnspan=5)
